...
namespace Physics
{
    ...
    public class ParticleSimulation
    {
        ...
        public void AddParticle(Particle p, ParticleMetadata metaData)
        {
            p.MetaData = metaData;
            _particles.Add(p);
        }
        ...
        public void AddParticleToParticle(Particle p, ParticleMetadata m, Particle p1, ConnectorMetadata cm)
        {
            AddParticle(p,m);
        ...
        public void RemoveParticle(Particle p)
        {
            ...
            {
                DisposeParticleMetaData(this, new MetaDataDisposeEventArgs(p.MetaData));
            }
            _particles.Remove(p);
        ...
        private void AccumulateForces()
        {
            foreach (Particle p in _particles)
            {
                ...
                foreach (Particle op in _particles)
                {
                    if (op!=p)
                    {
